...
export default {
  ...
  computed: {
    ...mapState(houseStore, ["houses", "center", "gugunAvgs"]),
  ...
  methods: {
    ...
    AllMarkerClear() {
      ...
      this.markerClear(this.gugunAvgMarkers);
    },
    ...
    moveGugun(lat, lng) {
      this.map.setLevel(4);
      this.markerClear(this.gugunAvgMarkers);
      this.SET_MAP_CENTER({ lat: lat, lng: lng });
    },
    ...
    setAptsOnMap() {
      ...
      this.houses.forEach((house) => {
        ...
        kakao.maps.event.addListener(marker, "click", () => {
          this.houseInfoExp(house);
          this.detailHouse(house);
          this.gethouseDealChart(house.aptCode);
          this.getNearObject({ lng: house.lng, lat: house.lat });
          this.SET_MAP_CENTER({ lat: house.lat, lng: house.lng });
        });
      });
    },
    setGugunAvgOnMap() {
      this.gugunAvgs.forEach((avg) => {
        let div = document.createElement("div");
        div.style.width = "85px";
        div.style.height = "53px";
        div.style["background-image"] =
          "url(https://apis.zigbang.com/marker/v6/local?mode=item&nameSize=1&level=up5&select=n&dpi=160)";
        div.style["text-align"] = "center";
        div.style["padding-top"] = "8px";
        div.style.cursor = "pointer";
        div.onclick = () => {
          this.moveGugun(avg.lat, avg.lng);
        };

        let name = document.createElement("div");
        name.style["font-size"] = "11px";
        name.style["color"] = "rgb(254, 254, 254)";
        name.appendChild(document.createTextNode(avg.gugunName));

        let amount = document.createElement("div");
        amount.style["font-size"] = "11px";
        amount.style["color"] = "rgb(254, 254, 254)";
        amount.appendChild(document.createTextNode(avg.amountAvg + "억"));

        div.appendChild(name);
        div.appendChild(amount);

        let position = new kakao.maps.LatLng(avg.lat, avg.lng);
        ...
        this.gugunAvgMarkers.push(customOverlay);
      });
    },
    setHospitalOnMap() {
  ...
  watch: {
    ...
    latlng: {
      deep: true,
      handler() {
        if (this.map.getLevel() > 8) return;
        if (this.map.getLevel() > 5) {
          this.getGugunAvg({ latlng: this.latlng, filter: this.getFilter });
          return;
        }

        this.getHouseListRange({ latlng: this.latlng, filter: this.getFilter });
        if (this.isHospital) this.getHospitaListRange(this.latlng);
        if (this.isSchool) this.getSchoolListRange(this.latlng);
        if (this.isBusStation) this.getBusStationListRange(this.latlng);
        if (this.isSubway) this.getSubwayListRange(this.latlng);
